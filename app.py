...
app.config['SECRET_KEY'] = 'asl_translator_secret_key'
...
def handle_frame(data):
    ...
    # Process the frame
    annotated_frame, gesture, confidence, nod_detected, calibration = process_frame(frame, session_id)
...
def handle_export_translations():
    ...
    if session_id in client_sessions:
        ...
        emit('export_result', {
            'text': export_text,
            'filename': f"ASL_Translation_{time.strftime('%Y-%m-%d')}.txt"
        })
